...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive) {
  ...
  var request =
   ...
   this.options.entryPoint + (isPassive ? "\" IsPassive=\"true" : "") +  "\">" +
...
function validateResponse(self, doc, callback) {
    ...
    if (response) {
      ...
      if (!assertion) {
        ...
        if (status) {
          status = self.getElement(status[0], 'StatusCode');
      ...
      function attrValueMapper(value) {
...
function validateRequest(self, doc, callback) {
    var request = self.getElement(doc, 'LogoutRequest');
    if (request) {
      var profile = {};
      if (request.$.ID) {
      ...
      var issuer = self.getElement(request, 'Issuer');
      if (issuer) {
        profile.issuer = issuer[0];
      } else {
      ...
      var nameID = self.getElement(request, 'NameID');
      if (nameID) {
        profile.nameID = nameID[0]._;
        if (nameID[0].$.Format) {
          profile.nameIDFormat = nameID[0].$.Format;
